...
public class SqlTest {
    ...
    public void testSelect() {
        String string = new SelectBuilder().select(User::getId, "id1")
                ...
                .eq(User::getName, "aa")
                .and(e -> e.like(User::getName, "1").ge(User::getId, 1).or(c -> c.isNotNull(User::getId).eq(User::getId, 1)))
                .or(e -> e.eq(User::getId, 1).eq(User::getId, 2))
                .leftJoin(User::getId, User::getAge, join -> join.notIn(User::getName, "bb").or(e -> e.eq(User::getId, 1).between(User::getId, 2, 3)))
